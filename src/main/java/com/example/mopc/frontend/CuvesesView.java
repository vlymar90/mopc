...
@Route("cuveses")
public class CuvesesView extends VerticalLayout{
    private final Grid<Cuveses> grid = new Grid<>(Cuveses.class);
    private final CuvesesService cuvesesService;
    private TextField searchField;
    private List<Cuveses> apparatusList;
    private Authentication authentication;

    CuvesesView(CuvesesService cuvesesService){
        setSizeFull();
    ...
    private HorizontalLayout headerButton() {
        ...
        Button mistake = new Button("Сломанные", item -> {
            grid.setItems(cuvesesService.getAllMistake());
        });
        vl.add(mistake);
    ...
    private HorizontalLayout searchComponent() {
        ...
        Button searchButton = new Button("Поиск", item -> {
            ...
            apparatusList = cuvesesService.getAllLikeSerialNumber(searchField.getValue());
    ...
    private void initGrid() {
        ...
        if (apparatusList.size() == 0) {
            apparatusList = cuvesesService.getAll();
        }
        if (apparatusList != null && apparatusList.size() != 0) {
            ...
            if(authentication.getName().equals("admin")) {
                grid.addColumn(new ComponentRenderer<>(item -> {
                    var changeButton = new Button("Удалить", i -> {
                        cuvesesService.remove(item.getId());
            ...
            ListDataProvider<Cuveses> dataProvider = DataProvider.ofCollection(apparatusList);
    ...
    private Dialog showDialog(Cuveses app) {
        Dialog d = new Dialog();
        if (app != null) {
            Text title = new Text("Устройство SN: " + app.getSerial());
            ...
            Button save = new Button("Изменить", item -> {
                Cuveses apparatus = cuvesesService.getEntity(app.getId());
                apparatus.setWorking(isWorked.getValue());
                apparatus.setComment(comment.getValue());
                cuvesesService.save(apparatus);
                UI.getCurrent().getPage().reload();
                d.close();
            });
